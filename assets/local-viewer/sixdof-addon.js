...
(async function installSixDofLab(){
  ...
  const xyzRanges=[0,1,2].map(axis=>range(paths.layers.raw.map(p=>p[axis])));
  ...
  function setBar(axis,value){const[lo,,span]=xyzRanges[axis],ratio=Math.max(0,Math.min(1,(value-lo)/span));document.getElementById(`bar-${"xyz"[axis]}`).style.width=`${ratio*100}%`;}
  ...
  function update(){const index=Math.max(0,Math.min(cameras.samples.length-1,Number(slider.value)||0)),pose=selectedPose(index),position=pose.position,euler=quaternionEuler(orientationForDisplay(pose.quaternion_wxyz,pose.orientation_convention)),previousPose=index?selectedPose(index-1):pose,previous=quaternionEuler(orientationForDisplay(previousPose.quaternion_wxyz,previousPose.orientation_convention)),dt=index?Math.max(pose.timestamp_sec-previousPose.timestamp_sec,1e-6):1,angular=Math.hypot(angleDelta(euler.roll,previous.roll),angleDelta(euler.pitch,previous.pitch),angleDelta(euler.yaw,previous.yaw))/dt;
    ["x","y","z"].forEach((axis,i)=>{document.getElementById(`six-${axis}`).textContent=position[i].toFixed(4);setBar(i,position[i]);});document.getElementById("six-roll").textContent=`${euler.roll.toFixed(1)}°`;document.getElementById("six-pitch").textContent=`${euler.pitch.toFixed(1)}°`;document.getElementById("six-yaw").textContent=`${euler.yaw.toFixed(1)}°`;document.getElementById("six-speed").textContent=Number(profiles.speed_relative[index]).toFixed(4);document.getElementById("six-accel").textContent=Number(profiles.acceleration_relative[index]).toFixed(4);document.getElementById("six-jerk").textContent=Number(profiles.jerk_proxy[index]).toFixed(4);document.getElementById("six-angular").textContent=`${angular.toFixed(1)}°/s`;document.getElementById("six-state").textContent=`${selector.options[selector.selectedIndex].text} · sample ${index+1}/${cameras.samples.length} · ${pose.timestamp_sec.toFixed(3)} source s`;drawAttitude(euler);
  }
  slider.addEventListener("input",update);selector.addEventListener("change",update);addEventListener("resize",update);update();
})();
