...
(async function installReferenceSceneControls(){
  ...
  function drawOverlay(){if(document.body.classList.contains("webgl-ready"))return;const ctx=overlay.getContext("2d"),{w,h}=sizeCanvas(overlay);ctx.clearRect(0,0,w,h);const b=meta.bounds,span=Math.max(...b.max.map((v,i)=>v-b.min[i]),1e-6);
