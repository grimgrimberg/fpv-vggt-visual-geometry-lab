...
function transformPosition(position){const shared=window.FPVViewer?.toDisplayPoint?.(position);if(Array.isArray(shared)&&shared.length===3&&shared.every(Number.isFinite))return shared.map(Number);const transform=activeDisplayTransform();if(!transform)return position.map(Number);const r=transform.rotation_3x3,t=transform.translation;return r.map((row,axis)=>row.reduce((sum,value,index)=>sum+value*Number(position[index]),Number(t[axis])));}
...
(async function installSixDofLab(){
  ...
  const segmentStarts=Array.isArray(paths.segment_boundaries)&&paths.segment_boundaries.length===cameras.samples.length?paths.segment_boundaries.map(Boolean):cameras.samples.map((_,index)=>index===0);
  ...
  function update(){const index=Math.max(0,Math.min(cameras.samples.length-1,Number(slider.value)||0)),pose=displayPose(selectedPose(index)),position=pose.position,euler=quaternionEuler(pose.quaternion_wxyz),atBoundary=segmentStarts[index]===true,previousPose=index&&!atBoundary?displayPose(selectedPose(index-1)):pose,previous=quaternionEuler(previousPose.quaternion_wxyz),dt=index&&!atBoundary?Math.max(pose.timestamp_sec-previousPose.timestamp_sec,1e-6):1,angular=atBoundary?null:Math.hypot(angleDelta(euler.roll,previous.roll),angleDelta(euler.pitch,previous.pitch),angleDelta(euler.yaw,previous.yaw))/dt,displayPositions=cameras.samples.map((_,sampleIndex)=>displayPose(selectedPose(sampleIndex)).position),xyzRanges=[0,1,2].map(axis=>range(displayPositions.map(point=>point[axis]))),displayFrame=activeDisplayTransform()?"estimated-ground display":"raw reconstruction display";
    ["x","y","z"].forEach((axis,i)=>{document.getElementById(`six-${axis}`).textContent=position[i].toFixed(4);setBar(i,position[i],xyzRanges);});document.getElementById("six-roll").textContent=`${euler.roll.toFixed(1)}°`;document.getElementById("six-pitch").textContent=`${euler.pitch.toFixed(1)}°`;document.getElementById("six-yaw").textContent=`${euler.yaw.toFixed(1)}°`;document.getElementById("six-speed").textContent=Number(profiles.speed_relative[index]).toFixed(4);document.getElementById("six-accel").textContent=Number(profiles.acceleration_relative[index]).toFixed(4);document.getElementById("six-jerk").textContent=Number(profiles.jerk_proxy[index]).toFixed(4);document.getElementById("six-angular").textContent=angular===null?"— edit boundary":`${angular.toFixed(1)}°/s`;document.getElementById("six-state").textContent=`${selector.options[selector.selectedIndex].text} · ${displayFrame} · ${cameras.samples[index].segment_id||"segment"} · sample ${index+1}/${cameras.samples.length} · ${pose.timestamp_sec.toFixed(3)} source s`;drawAttitude(euler);
  }
